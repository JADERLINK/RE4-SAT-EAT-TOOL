...
namespace RE4_SAT_EAT_EXTRACT
{
    public static class Debug
    {
        ...
        public static void EsatLinesOBJ(FileInfo info, ESAT esat, bool switchStatus) 
        {
            ...
            {
                float x = esat.Edges[i].X / 100f;
                float y = esat.Edges[i].Y / 100f;
                float z = esat.Edges[i].Z / 100f;
        ...
        public static void EsatGroupBoxOBJ(FileInfo info, ESAT esat) 
        {
            ...
            for (int i = 0; i < esat.Groups.Length; i++)
            {
                ...
                text.WriteLine("v " + 
                    ...
                    (bound_pos.Y / 100f).ToFloatString() + " " +
                    (bound_pos.Z / 100f).ToFloatString());

                //soma dos 3 -- G
                text.WriteLine("v " + 
                    ((bound_pos.X + bound_dim.X) / 100f).ToFloatString() + " " +
                    ((bound_pos.Y + bound_dim.Y) / 100f).ToFloatString() + " " +
        ...
        public static void EsatGroupArrowOBJ(FileInfo info, ESAT esat) 
        {
            ...
            for (int i = 0; i < esat.Groups.Length; i++)
            {
                ...
                text.WriteLine("v " + 
                    ((bound_pos.X + 1000) / 100f).ToFloatString() + " " +
                    ((bound_pos.Y + climbHeight + 1000) / 100f).ToFloatString() + " " +
                    (bound_pos.Z / 100f).ToFloatString());
